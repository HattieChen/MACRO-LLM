...
log_filename = os.path.join(_log_dir, f"{timestamp}-2.log")
logging.basicConfig(
...
class CACCWrapper(gym.Wrapper):
    def __init__(self, config_path, bias=0, std=1, test=False):
        ...
        self.std=std
        self.test = test
        # align wrapper agent count with underlying env config
    ...
    def step(self, action):
        ...
        if type(action[0]) == list:
            action = action[0]
        logging.info(f"----------- 1. state: {self.state}, action: {action}")
        state, reward, done, info = self.env.step(action)
        if self.test:
            reward = self._comparable_reward()
        state = np.array(state, dtype=np.float32)
        reward = np.array(reward, dtype=np.float32)
        done = np.array([done]*self.n_agents, dtype=np.float32)
        # done = np.array([done]*8, dtype=np.float32)
        self.state=state
        reward = (reward+self.bias)/self.std
        # TODO:
        logging.info(f"----------- 2. next_state: {self.state}, reward: {reward}")
        #print("reward=",reward)
        return state, np.clip(reward, -5, 5), done, None

    def llm_step(self, action):
        logging.info(f"----------- 1. state: {self.state}, action: {action}")
        state, reward, done, info = self.env.llm_step(action)
        if self.test:
            reward = self._comparable_reward()
        state = np.array(state, dtype=np.float32)
        reward = np.array(reward, dtype=np.float32)
